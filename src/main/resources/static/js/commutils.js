...
function sumitData(url,args) {
    $.post(url,args,function(retData){
        if(retData.code==0){
            ...
            toast("error","温馨提示",retData.msg);
            return;
        }
    },"JSON");
}

function submitData_v(url,args) {
    $.ajax({
        url: url,
        type: 'POST',
        ...
        success: function (retData) {
            if(retData.code==0){
                ...
                toast("error",retData.msg,retData.detail);
                return;
            }
...
function delConfirm(url,args,reqUrl) {
    swal({
            title: "您确定要删除吗",
            text: "删除后将无法恢复，请谨慎操作！",
            type: "warning",
        ...
        function (isConfirm) {
            if (isConfirm) {
                $.post(url,args,function (retData) {
                    if(retData.code==1){
                        swal("删除失败", retData.msg, "error");
                        return;
                    }else {
                        swal("操作成功！", "该记录已删除。", "success");
                        setTimeout(function(){
                            requestUrl(reqUrl);
                        },1000);
                    }
 ...
 * @param allowmaxSize
 * @returns {boolean}
 * typ 为判断类型 1表示大于 0 表示小于
 */
function checkFileSize(file,allowmaxSize,type) {

    //大小 字节
    var allM = Math.ceil(allowmaxSize/(1024*1024)); //计算单位为M
    var size=file.size
    if(type==0&&allowmaxSize<=size){
        Lobibox.alert('error', {
            title: title,
            position:"top center",
            msg: '上传文件必须小于'+allM+"M"
...
function getFileType(typeNum) {
    ...
    var ztpicture = new Array("jpg","png");
    //后续需要的类型直接往里面添加就行
    var types = new Array(type0,type1,fileType,ztpicture);
...
function toast(type,tilte,message) {
    if(tilte==undefined||tilte==''){
        tilte="温馨提示"
    }
    ...
    if(type=='error'){
        toastr.error(message,tilte);
    }
    if(type=='success'){
        toastr.success(message,tilte);
    }
}
...
function checkInput(obj,tip) {
    var inputValue=obj.val();
    if(inputValue==''||inputValue==undefined){
        toast("error",'温馨提示',tip);
        return false;
    }
    return true;
}
